...
const keySelection = document.querySelector('.piano_keys');

const playNote = (note) => {
    const audio = new Audio(`../audios/notes_${note}.wav`);
    audio.play();
};
...
checkbox.addEventListener('change', ({ target }) => {
    if (target.checked) {
        ...
        keySelection.classList.remove('disabled-keys');
    } else {
        switcher.classList.remove('switcher-active');
        keySelection.classList.add('disabled-keys');
    }
});
